...
public class GadgetWave : Gadget {
...
//		}
	...
	public override GameObject DropOneAmmo(Ammo a){
		if (numberHeld){
			...
			UpdateAmmoGraphics();
			ret.SetActive(true);
			ret.transform.position = GetAmmoDropPosition();
			return ret;
		} else return null;
	}
...
//		Debug.Log("numberheld:"+numberHeld);
	...
	public override void Fire ()
	{
		...
		GameObject wave = (GameObject)Instantiate(waveFormPrefab,Player.inst.transform.position + Player.inst.transform.forward+Player.inst.transform.up,Player.inst.transform.rotation);
...
//			Debug.Log("ammograph:"+ammoGraphics);
			...
			ammoGraphics.transform.localScale = Vector3.one * 0.75f;

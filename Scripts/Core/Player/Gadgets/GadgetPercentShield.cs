...
public class GadgetPercentShield : Gadget {

	public GameObject percentShieldWavePrefab;
	...
	public override bool CanCollectItem( GameObject o){ 
		return (!numberHeld && IsValidAmmo(o));
	...
	public override bool IsValidAmmo(GameObject o) {
		...
		if (o.GetComponent<WaveAmmo>()) flag = true;
		return flag;
...
//		Debug.Log("numberheld:"+numberHeld);
	...
	public override void Fire ()
	{
		...
		float waveUpOffset = 2.5f;
		GameObject wave = (GameObject)Instantiate(percentShieldWavePrefab,Player.inst.transform.position + Player.inst.transform.forward + Player.inst.transform.up,Player.inst.transform.rotation);
		wave.GetComponent<WaveInfoPercentShield>().SetWaveInfo(numberHeld.GetComponent<NumberInfo>().fraction);
...
//			ammoGraphics.transform.localScale *= 0.3f;
			...
			ammoGraphics.transform.localPosition = new Vector3(0.4f,3.5f,1.8f); // = gadgetGraphics.transform.position + Vector3.up * 5;// new Vector3(.3f,.078f,-.1f);
			ammoGraphics.transform.localScale = Vector3.one * 1.25f;

...
public class Buoyancy : MonoBehaviour
{
	...
	private void Start()
	{
		...
		if (GetComponent<Collider>() == null)
		{
			...
			// commented Debug.LogWarning(string.Format("[Buoyancy.cs] Object \"{0}\" had no collider. MeshCollider has been added.", name));
		}
		...
		if (GetComponent<Rigidbody>() == null)
		{
			...
			// commented Debug.LogWarning(string.Format("[Buoyancy.cs] Object \"{0}\" had no Rigidbody. Rigidbody has been added.", name));
		}
...
//		// commented Debug.Log(string.Format("[Buoyancy.cs] Name=\"{0}\" volume={1:0.0}, mass={2:0.0}, density={3:0.0}", name, volume, GetComponent<Rigidbody>().mass, density));
	}
...
//		return ocean == null ? 0.0f : ocean.GetWaterHeightAtLocation(x, z);
		float ret = 0;
		ret = UltimateToonWaterC.inst.getHeightByPos(new Vector2(x,z));
		if (ret == null) ret = 0;
		return ret;
...
//		return 0.0f;
	...
	{
		if (LevelBuilder.inst.levelBuilderIsShowing || GameManager.inst.GameFrozen) return;
